...
def preprocess_sst2(examples, tokenizer):
    ...
    tokenized["sentence"] = examples["sentence"]  # Retain original column for evaluation
    return tokenized
...
def preprocess_nli(examples, tokenizer):
    ...
    tokenized["hypothesis"] = examples["hypothesis"]  # Retain original column for evaluation
    return tokenized
...
def _tokenize(inputs, targets, tokenizer):
    ...
    for i in range(len(inputs)):
        model_inputs["input_ids"][i] = (
            [tokenizer.pad_token_id] * (MAX_LENGTH - len(model_inputs["input_ids"][i]))
            + model_inputs["input_ids"][i]
        )
...
def load_and_preprocess_dataset(dataset_name="sst2", tokenizer=None):
    ...
    # Preprocess examples
    preprocess_function = lambda examples: preprocess_sst2(examples, tokenizer) if dataset_name == "sst2" else preprocess_nli(examples, tokenizer)
    processed_ds = ds.map(
        preprocess_function,
        batched=True,
        num_proc=1,
